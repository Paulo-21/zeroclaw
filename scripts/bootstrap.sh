...
Modes:
  Default mode installs ZeroClaw only (requires existing Rust toolchain).
...
Options:
  ...
  --build-first              Run `cargo build --release --locked` before install
  --skip-build               Deprecated no-op (kept for compatibility)
...
run_guided_installer() {
  ...
  if [[ "$os_name" == "Linux" ]]; then
    if prompt_yes_no "Install Linux build dependencies (build-essential/pkg-config/git/curl)?" "yes"; then
  ...
  if prompt_yes_no "Run a separate prebuild before install?" "no"; then
    BUILD_FIRST=true
  fi
...
if [[ "$SKIP_BUILD_COMPAT" == true ]]; then
  warn "--skip-build is deprecated and now a no-op. Default flow already skips a separate prebuild."
fi
...
if [[ "$BUILD_FIRST" == true ]]; then
  ...
  info "Skipping separate prebuild (default flow)."
fi
